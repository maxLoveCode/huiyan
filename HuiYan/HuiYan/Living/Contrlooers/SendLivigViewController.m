...
- (UIButton *)sureBtn{
    if (!_sureBtn) {
        ...
        self.sureBtn.frame = CGRectMake(kMargin, CGRectGetMaxY(self.tableView.frame) + 20, kScreen_Width - 2 * kMargin, 50);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        ...
        if (!selectBtn) {
            ...
            selectBtn.frame = CGRectMake(kScreen_Width / 2 - 150, kMargin,300,300);
            selectBtn.titleEdgeInsets = UIEdgeInsetsMake(100, -100, 0, 0);
            selectBtn.imageEdgeInsets = UIEdgeInsetsMake(- 90, 90, 0, 0);
            [selectBtn setImage:[UIImage imageNamed:@"selectedPic"] forState:UIControlStateNormal];
